...
export default function PlayerCard({ player, name, meta, votes, avatar, disabled, onVote }: PlayerCardProps) {
  const theme = THEME[player];

  return (
    <motion.article
      layout
      className={`relative flex min-h-[520px] flex-col justify-between overflow-hidden rounded-lg border border-white/10 bg-white/[0.035] p-4 ${theme.glow} backdrop-blur md:p-5`}
    >
      ...
      <div className="flex flex-1 items-center justify-center py-3">{avatar}</div>

      <button
        type="button"
        disabled={disabled}
        onClick={onVote}
        className={`group inline-flex h-14 w-full items-center justify-center gap-2 rounded-md ${theme.accent} px-5 text-base font-black text-white ${theme.glow} transition hover:brightness-110 active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-50`}
      >
        Vote {name.split(" ")[0]}
